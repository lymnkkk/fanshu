<?php
    ...
    $article_search=mysql_query("SELECT id,headerImg,title,content,cat,userId,createdAt FROM article WHERE id='{$_POST['articleId']}'") or die ('sql错误'.mysql_error());

    $articleArr=[];
    $userName=[];
    $article=[];
    $articleAll=[];
    if($article_info=mysql_fetch_array($article_search,MYSQL_ASSOC)){
        $articleArr=$article_info;
        ...
        if($user_info=mysql_fetch_array($user_search,MYSQL_ASSOC)){
            $userName=array('userName'=>$user_info['name']);
        }

        $article=array_merge($articleArr,$userName);
